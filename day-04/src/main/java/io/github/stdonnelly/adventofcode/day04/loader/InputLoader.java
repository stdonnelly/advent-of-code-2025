package io.github.stdonnelly.adventofcode.day04.loader;
...
public class InputLoader {
  ...
  /**
   * Takes the input from the resource named in {@link inFileName} and parses it into a {@link
   * FloorMap}
   *
   * @return A FloorMap from the file
   * @throws IOException if there is a problem loading the input.txt file, or if there is a problem
   *     reading the file
   */
  public FloorMap load() throws IOException {
    try (InputStream is = this.getClass().getClassLoader().getResourceAsStream(inFileName);
      ...
      return FloorMap.parse(reader.lines());
    }
  }
}
