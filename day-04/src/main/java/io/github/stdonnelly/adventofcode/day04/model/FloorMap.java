...
public record FloorMap(FloorSquare[][] squares) {
   ...
   *
   * @param input The input strings to parse. This takes a {@link Stream} because this parses the
   *     entire file, rather than just a line.
  ...
  private static FloorSquare[] parseRow(String input) throws IllegalArgumentException {
    return input
        .chars()
        .mapToObj(mapChar -> FloorSquare.parse((char) mapChar))
        .toArray(FloorSquare[]::new);
  }
