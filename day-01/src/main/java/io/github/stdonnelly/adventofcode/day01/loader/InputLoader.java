...
public class InputLoader {
  ...
  /**
   * Takes the input from the resource named in {@link IN_FILE_NAME} and parses it into a list of
   * instructions
   *
  ...
  public List<Instruction> load() throws IOException {
    try (InputStream is = this.getClass().getClassLoader().getResourceAsStream(inFileName);
      ...
      return reader.lines().map(Instruction::parse).collect(Collectors.toList());
    }
  }
}
