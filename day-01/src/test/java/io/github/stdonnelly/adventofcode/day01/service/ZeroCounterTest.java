...
class ZeroCounterTest {
  ...
  void countAllZerosDuringExecutionTest() throws IOException {
    ...
    assertEquals(
        EXPECTED_COUNT, zeroCounter.countAllZerosDuringExecution(new Dial(), instructions));
  }
  ...
  void countAllZerosDuringExecutionParameterizedTest(
      final String description, final String instructionsStr, final int expected) {

    List<Instruction> instructions;

    if (instructionsStr != null && !instructionsStr.isBlank()) {
      final String[] instructionsArray = instructionsStr.split(":");
      instructions = Arrays.stream(instructionsArray).map(Instruction::parse).toList();
    } else {
      instructions = Collections.emptyList();
    }

    assertEquals(
        expected, zeroCounter.countAllZerosDuringExecution(new Dial(), instructions), description);
  }
}
